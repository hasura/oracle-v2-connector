# Contributing to Hasura Oracle V2 Connector
...
This repository contains the source code of the Hasura GraphQL Engine DDN V2 Connector for the Oracle database.

It is a Kotlin project using Gradle as a build tool. The project is split into three modules:

- `gdc-ir`: This module contains the type definitions for the DDN Query IR
- `sql-gen`: This module contains implementation specific code for generating SQL from the DDN Query IR
- `app`: This module contains the Quarkus application that exposes the service and it's API

The `buildSrc` directory contains some project-wide configuration and dependencies that are used by all modules.
The `lib` directory contains some vendored Maven M2 dependencies that are not available in Maven Central.
